...
export default function CultureCard({ data, tunerSettings }) {
  ...
  const getBambooColor = (healthy, stressed) => {
    if (stressed) return 'from-amber-600 to-red-500';
    if (healthy) return 'from-emerald-400 to-green-600';
        ...
        <div className="absolute bottom-4 left-0 right-0 flex justify-around items-end px-4">
          {bambooData.map((bamboo, index) => (
            <div key={index} className="flex flex-col items-center">
              <motion.div
                initial={{ height: 0 }}
                animate={{ height: `${bamboo.height * 1.2}px` }}
                transition={{ duration: 0.8, delay: index * 0.1 }}
                className="relative w-4 rounded-t-full overflow-hidden"
                style={{ maxHeight: '120px' }}
              >
                {/* Bamboo Stalk */}
                <div className={`absolute inset-0 bg-gradient-to-t ${getBambooColor(bamboo.healthy, stressLevel > 0.6)}`} />
                ...
                {bamboo.healthy && (
                  <>
                    <motion.div
                      ...
                      className="absolute -top-1 -left-3 w-6 h-2 bg-green-500 rounded-full transform -rotate-45"
                    />
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      transition={{ delay: 0.9 + index * 0.1 }}
                      className="absolute -top-1 -right-3 w-6 h-2 bg-green-500 rounded-full transform rotate-45"
                    />
                  </>
                )}
              </motion.div>
