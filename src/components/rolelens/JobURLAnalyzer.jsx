...
export default function JobURLAnalyzer({ onJobDataLoaded, isLoading, setIsLoading, tunerSettings, onFallbackToManual }) {
  ...
  // Normalize LinkedIn URLs to point to a specific job view page when possible
  const normalizeLinkedInUrl = (inputUrl) => {
    try {
      const urlObj = new URL(inputUrl);
      if (!urlObj.hostname.includes('linkedin.com')) return inputUrl;
      
      // Handle linkedin.com/jobs/collections/...?currentJobId=123
      // and linkedin.com/jobs/search/...?currentJobId=123
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // Step 1: Normalize URL (e.g. LinkedIn collections → direct job view)
      const normalizedUrl = normalizeLinkedInUrl(url.trim());

      // Step 2: Fetch the actual page content via LLM web access
      setCurrentStep(0);
      const pageContent = await fetchJobPage(normalizedUrl);

      // Step 3: Extract structured job data from page text
      setCurrentStep(1);
      const extractedData = await extractJobData(pageContent, normalizedUrl);

      // Step 4: Run full company + role analysis
      setCurrentStep(2);
      const analysisResult = await runFullAnalysis(extractedData);

      // Build a unified parsedJSON for downstream intelligence enrichment
      const companyName = extractedData.company || analysisResult.meta?.company || 'Unknown Company';
      const jobTitle = extractedData.title || analysisResult.meta?.title || 'Unknown Role';
      const location = extractedData.location || analysisResult.meta?.location || 'Unknown';

      // Convert to intelligence JSON shape
      const parsedJSON = {
        generated_at: new Date().toISOString(),
        company_name: companyName,
        role_analyzed: jobTitle,
        analysis_status: 'complete',
        dimensions: {
          compensation: {
            score: 60,
            market_low: analysisResult.comp?.range_min || analysisResult.comp?.base || 0,
            market_median: analysisResult.comp?.headline || 0,
            market_high: analysisResult.comp?.range_max || (analysisResult.comp?.headline ? Math.round(analysisResult.comp.headline * 1.2) : 0),
            headline: `$${(analysisResult.comp?.headline || 0).toLocaleString()} estimated`,
            insight: analysisResult.comp?.leak_label || '',
            confidence: 'medium',
            sources: ['AI Web Research']
          }
        },
        company_health: {
          stability_label: analysisResult.stability?.health || 'Unknown',
          headcount_trend: analysisResult.stability?.headcount_trend || 'unknown',
          financial_health_score: analysisResult.stability?.risk_score != null ? Math.round((1 - analysisResult.stability.risk_score) * 100) : 50
        },
        news: analysisResult.sources?.map(s => ({
          headline: s.title,
          url: s.url,
          source: s.publisher,
          sentiment: 'neutral'
        ...
        key_takeaways: [],
        action_items: { questions_to_ask: [], negotiation_points: [], research_needed: [] },
        data_sources_used: ['AI Web Research'],
        disclaimer: 'Analysis generated by AI. Verify critical information independently.'
      };

      // Step 5: Enhance with real API data (financial intelligence)
      setCurrentStep(3);
      ...
      try {
        ...
        const existingCompanyRecord = findMatchingCompany(companyName, allCompanyRecords);
        ...
        if (realIntelRes.data?.success && realIntelRes.data?.dimensions) {
          ...
          if (realIntelRes.data.newsArticles?.length > 0) {
      ...
      if (parsedJSON.news?.length > 0) companyUpdateData.news_articles = parsedJSON.news;
      ...
      companyId = await upsertPublicCompanyData(base44.entities.PublicCompanyData, companyName, companyUpdateData);

      await upsertJobApplication(base44.entities.JobApplication, companyName, jobTitle, {
        job_title: jobTitle,
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="pl-10 pr-24 h-12 outline-none focus:ring-0"
